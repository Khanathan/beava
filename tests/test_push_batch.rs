...
/// Phase 43 T4: handle_push_batch must fail-loud-reject every event in
/// the batch with a `server_busy:` prefix when `at_memory_ceiling` is
/// set. This is the signal the Python SDK watches for to raise
/// ServerBusyError (a distinct type from transport ConnectionError so
/// the user-facing retry policy does not hammer the overloaded server).
#[test]
fn handle_push_batch_rejects_when_at_memory_ceiling() {
    let state = make_state();
    register(&state, vec![count_stream("Tx", "user_id")]);
    // Flip the reject-writes gate as if the periodic timer had seen
    // RSS cross BEAVA_MEMORY_LIMIT_MB.
    state
        .at_memory_ceiling
        .store(true, std::sync::atomic::Ordering::Relaxed);

    let batch: Vec<PendingAsync> = (0..3)
        .map(|i| {
            PendingAsync::new(
                i,
                "Tx".into(),
    ...
    for (i, r) in results.iter().enumerate() {
        let err = r
            .as_ref()
            .err()
            .unwrap_or_else(|| panic!("event {} should have errored under memory ceiling", i));
        let msg = err.to_string();
        assert!(
            msg.starts_with("Protocol error: server_busy:")
                || msg.contains("server_busy:"),
            "error {} should carry 'server_busy:' marker, got: {}",
            i,
            msg
        );
    }

    // When the ceiling clears, batches flow again.
    state
        .at_memory_ceiling
    ...
    for (i, r) in results.iter().enumerate() {
        assert!(
            r.is_ok(),
            "after clearing ceiling, event {} should succeed; got {:?}",
            i,
            r
        );
    }
