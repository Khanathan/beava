...
fn push_purchase(
    ...
    let wm_opt = engine.watermarks.read().watermark("Purchases");
    if let Some(wm) = wm_opt {
        if event_time < wm {
            engine.late_drops.write().increment("Purchases");
            return;
        }
    }
    engine.watermarks.write().observe("Purchases", event_time);
...
fn integ_full_dag_in_order_happy_path() {
    ...
    let wm = engine.watermarks.read().watermark("Purchases").unwrap();
    assert_eq!(wm, t0 + Duration::from_secs(102) - WATERMARK_LATENESS);

    // No late drops.
    assert_eq!(engine.late_drops.read().get("Purchases"), 0);

    // γ propagation: PurchasesAgg.wm attached from Purchases.wm.
    let agg_wm = engine.watermarks.read().watermark("PurchasesAgg");
    assert_eq!(
        agg_wm, Some(wm),
...
fn integ_out_of_order_within_5s_lands_in_bucket() {
    ...
    assert_eq!(
        engine.late_drops.read().get("Purchases"),
        0,
    ...
    assert_eq!(
        engine.watermarks.read().observed_max("Purchases"),
