...
const BIAS_DATA: [f64; 201] = [
    2953.0, 2900.4782, 2848.3568, 2796.3666, 2745.324, 2694.9598, 2644.648, 2595.539,
    2546.1474, 2498.2576, 2450.8376, 2403.6076, 2357.451, 2311.38, 2266.4104, 2221.5638,
    2176.9676, 2134.193, 2090.838, 2048.8548, 2007.018, 1966.1742, 1925.4482, 1885.1294,
    1846.4776, 1807.4044, 1768.8724, 1731.3732, 1693.4304, 1657.5326, 1621.949, 1586.5532,
    1551.7256, 1517.6182, 1483.5186, 1450.4528, 1417.865, 1385.7164, 1352.6828, 1322.6708,
    1291.8312, 1260.9036, 1231.476, 1201.8652, 1173.6718, 1145.757, 1119.2072, 1092.2828,
    1065.0434, 1038.6264, 1014.3192, 988.5746, 965.0816, 940.1176, 917.9796, 894.5576,
    ...
    563.834, 547.363, 531.5066, 520.4554, 505.5832, 488.366, 476.4808, 459.7682,
    450.0522, 434.3288, 423.9528, 408.727, 399.0794, 387.2522, 373.988, 360.852,
    351.6394, 339.642, 330.9024, 322.6616, 311.6622, 301.3254, 291.7484, 279.9392,
    276.7508, 263.2152, 254.8114, 245.5494, 242.3064, 234.8734, 223.7872, 217.7156,
    212.0196, 200.793, 195.9748, 189.0702, 182.4492, 177.2772, 170.2336, 164.741,
    158.6136, 155.311, 147.5964, 142.837, 137.3724, 132.0162, 130.0424, 121.9804,
    120.4518, 114.8968, 111.586, 105.9332, 101.705, 98.5142, 95.0488, 89.788,
    91.475, 83.7764, 80.9698, 72.8574, 73.1616, 67.5838, 62.6264, 63.2638,
    66.0978, 52.0844, 58.9956, 47.0912, 46.4956, 48.4384, 47.1082, 43.2392,
    37.276, 40.0284, 35.1864, 35.8596, 32.0998, 28.027, 23.6694, 33.8266,
    26.3736, 27.2008, 21.3246, 26.4116, 23.4522, 19.5014, 19.8514, 10.7492,
    18.6424, 13.1266, 18.2436, 6.7186, 3.3946, 6.3376, 7.7672, 0.814,
    3.8282, 0.8262, 8.0744, -1.5908, 5.0178, 0.4554, -0.242, 0.1748,
    -9.0764, -4.2016, -3.7752, -4.7518, -5.3724, -8.9068, -6.1024, -5.7412,
    -9.9534, -3.8634, -13.7304, -16.271, -7.5136, -3.3068, -13.134, -10.0552,
    -6.7202, -8.5966, -10.9308, -1.8776, -4.8226, -13.7788, -21.647, -10.6736,
    -15.78,
];
...
impl Default for Hll {
    fn default() -> Self {
        Hll { phase: HllPhase::Exact(Vec::new()) }
    }
}
...
impl Hll {
    ...
    fn estimate_bias_knn(raw: f64) -> f64 {
        ...
        let mut distances: Vec<(f64, usize)> = RAW_ESTIMATE_DATA.iter()
            .enumerate()
        ...
        let k = KNN_K.min(n);
        let bias_sum: f64 = distances[..k].iter()
            .map(|&(_, idx)| BIAS_DATA[idx])
            .sum();
        bias_sum / k as f64
    }
...
mod tests {
    ...
    fn test_hashset_to_dense_promotion() {
        ...
        assert!(count >= 1900.0 && count <= 2100.0,
            "Expected ~2000, got {}", count);
    }
    ...
    fn test_dense_10000_unique() {
        ...
        assert_eq!(h.phase_name(), "hll");
        let count = h.count();
        assert!(count >= 9500.0 && count <= 10500.0,
    ...
    fn test_merge_exact_exact() {
        let mut a = Hll::new();
        let mut b = Hll::new();
        for i in 0..5 { a.insert(&format!("a_{}", i)); }
        for i in 0..5 { b.insert(&format!("b_{}", i)); }
        a.merge(&b);
    ...
    fn test_merge_exact_promotes_to_hashset() {
        let mut a = Hll::new();
        let mut b = Hll::new();
        for i in 0..10 { a.insert(&format!("a_{}", i)); }
        for i in 0..10 { b.insert(&format!("b_{}", i)); }
        a.merge(&b);
    ...
    fn test_merge_hashset_hashset() {
        let mut a = Hll::new();
        let mut b = Hll::new();
        for i in 0..100 { a.insert(&format!("a_{}", i)); }
        for i in 0..100 { b.insert(&format!("b_{}", i)); }
        a.merge(&b);
    ...
    fn test_merge_cross_phase_exact_into_dense() {
        ...
        for i in 0..2000 { dense.insert(&format!("d_{}", i)); }
        for i in 0..5 { exact.insert(&format!("e_{}", i)); }
    ...
    fn test_memory_fraud_typical_user() {
        ...
        assert_eq!(h.phase_name(), "exact");
        assert!(h.size_bytes() <= 40, "5 merchants should be ≤ 40 bytes, got {}", h.size_bytes());
    }
    ...
    fn test_memory_fraud_active_merchant() {
        ...
        assert_eq!(h.phase_name(), "hashset");
        assert!(h.size_bytes() <= 1700, "200 users should be ≤ 1.7 KB, got {}", h.size_bytes());
    }
    ...
    fn test_postcard_round_trip_exact() {
        let mut h = Hll::new();
        for i in 0..10 { h.insert(&format!("item_{}", i)); }
        let before = h.count();
        ...
        assert!(bytes.len() < 200, "Exact 10 items should serialize small, got {}", bytes.len());
    }

    #[test]
    fn test_postcard_round_trip_hashset() {
        let mut h = Hll::new();
        for i in 0..100 { h.insert(&format!("item_{}", i)); }
        let before = h.count();
        let bytes = postcard::to_allocvec(&h).unwrap();
        let restored: Hll = postcard::from_bytes(&bytes).unwrap();
        assert_eq!(before, restored.count());
    ...
    fn test_postcard_round_trip_dense() {
        let mut h = Hll::new();
        for i in 0..5000 { h.insert(&format!("item_{}", i)); }
        let before = h.count();
        let bytes = postcard::to_allocvec(&h).unwrap();
        let restored: Hll = postcard::from_bytes(&bytes).unwrap();
    ...
    fn make_op(optional: bool) -> DistinctCountOp {
        DistinctCountOp::new("merchant_id", Duration::from_secs(5 * 60), Duration::from_secs(60), optional)
    }
    ...
    fn test_dc_5_unique() {
        ...
        let t0 = ts(60000);
        for i in 0..5 {
            op.push(&serde_json::json!({"merchant_id": format!("m{}", i)}), None, t0).unwrap();
        }
        match op.read(t0) {
    ...
    fn test_dc_duplicates() {
        ...
        let t0 = ts(60000);
        for _ in 0..5 {
            op.push(&serde_json::json!({"merchant_id": "same"}), None, t0).unwrap();
        }
        match op.read(t0) {
    ...
    fn test_dc_expiry() {
        let mut op = make_op(false);
        let t0 = ts(60000);
        op.push(&serde_json::json!({"merchant_id": "m1"}), None, t0).unwrap();
        assert_ne!(op.read(t0), FeatureValue::Missing);
        assert_eq!(op.read(t0 + Duration::from_secs(600)), FeatureValue::Missing);
    }
    ...
    fn test_dc_cross_bucket_merge() {
        ...
        let t0 = ts(60000);
        for i in 0..3 {
            op.push(&serde_json::json!({"merchant_id": format!("m{}", i)}), None, t0 + Duration::from_secs(i * 60)).unwrap();
        }
    ...
    fn test_dc_optional_skips_absent() {
        ...
        assert!(op.push(&serde_json::json!({"other": "val"}), None, ts(60000)).is_ok());
        assert_eq!(op.read(ts(60000)), FeatureValue::Missing);
    }
    ...
    fn test_dc_required_errors_on_absent() {
        let mut op = make_op(false);
        assert!(op.push(&serde_json::json!({"other": "val"}), None, ts(60000)).is_err());
    }
    ...
    fn test_dc_numeric_values() {
        ...
        let t0 = ts(60000);
        for i in 0..5 {
            op.push(&serde_json::json!({"merchant_id": i}), None, t0).unwrap();
        }
        match op.read(t0) {
