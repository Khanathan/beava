...
pub fn parse_duration_str(s: &str) -> Result<std::time::Duration, BeavaError> {
    let s = s.trim();
    if s.is_empty() {
        return Err(BeavaError::Protocol("empty duration string".into()));
    }
    ...
    if let Some(num_str) = s.strip_suffix("ms") {
        let millis: u64 = num_str
            .parse()
            .map_err(|_| BeavaError::Protocol(format!("invalid duration number: {}", s)))?;
