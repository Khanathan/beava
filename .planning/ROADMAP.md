...
| 18 | Redis-shaped hand-rolled hot path | Replace tokio on apply + wire path with hand-rolled event loop matching Redis 7.x architecture: pthread-owned RefCell apply state, `mio`/`io_uring` I/O threads (Redis 6.0 pattern), inline WAL with off-thread fsync, `httparse`-based hand-rolled HTTP/1.1 + framed-TCP parser sharing one apply thread. Stages 18.1–18.6 with informational M4 perf gates and HARD Linux-Xeon ship-gate ≥3M EPS/core simple-fraud TCP at Stage 18.5. Admin endpoints stay on tokio/axum on port 8081. | ~4000 LoC | 📋 **PLANNED** (10/10 planning docs landed 2026-04-24; executor pending upstream merge round) |
