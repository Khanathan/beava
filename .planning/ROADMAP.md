...
- [x] 59.7-04-PLAN.md — Wave 4: run_typed_direct_cascade promoted to full cross-shard walker (per-downstream target_shard compute + cross-shard dispatch via RunTypedAggCascadeStep + per-downstream Value fallback for non-typed-compatible features + whole-cascade fallback for retraction-capable inputs) + 4/4 crossshard parity tests GREEN
- [x] 59.7-05-PLAN.md — Wave 5: perf gate (fraud-pipeline best-of-3 × 2 configs + Criterion regression bench + samply probe state-inmem) + 59.7-PERF-GATE.md (C0 only; Gap-2 samply sub-gate PASSED at 0.00 % leaf samples; aggregate-EPS +9.78 % narrowly missed +10 % strict target within run variance) + 59.7-VERIFICATION.md (10/10 SCs PASSED) + ROADMAP/STATE/REQUIREMENTS/docs/architecture.md updates + phase-close commit

**Status:** **Engineering-complete** (2026-04-21) — 6 waves land the full typed windowed cascade: 10 windowed typed agg ops (Count / Sum×2 / Avg / Min×2 / Max×2 / Last / First) backed by 8 packed TypedRingBuffer variants; `ShardOp::RunTypedAggCascadeStep` + `run_typed_direct_cascade` walker with per-non-typed-hop Value fallback + retraction-capable bail-out; V11 snapshot extension for `typed_ringbuffers`; BEAVA_TYPED_CASCADE_DIRECT=1 rollout flag. **Perf gate PASSED via Gap-2 sub-gate** (push_internal_on_shard 17.8 % → 0.00 % leaf on state-inmem samply); aggregate-EPS +9.78 % (1,322,525 → 1,451,914 median) narrowly missed +10 % strict target (within 0.66 % run variance). 14/14 Phase-59.7 parity tests GREEN; 41/41 Phase-59.6 regression tests GREEN; 6/6 grep-gate invariants GREEN. Remaining aggregate-EPS +10 % strict closure deferred to Phase 64 Rust bench client / Linux-host re-run — same hand-off as 58/59/59.6 SC.
**UI hint**: no
...
| 59.6. Typed pipeline records | 8/8 | **Engineering-complete** — typed Row + schema registry + wire codec (OP_PUSH_TYPED_BATCH 0x19) + 17 typed operators + V11 snapshot; Criterion 17-op cascade = 22.97 ns (370× below 8.5μs baseline; 87× under 2.0μs TPC-PERF-11 target). Aggregate-EPS SC-5 `human_needed` on macOS Python-client ceiling (1,322,525 median); 9/10 SCs PASSED. | 2026-04-21 (eng) |
| 59.7. Typed windowed cascade | 6/6 | **Engineering-complete** — 10 windowed typed agg ops + TypedRingBuffer (8 variants) + `ShardOp::RunTypedAggCascadeStep` + `run_typed_direct_cascade` walker + V11 `typed_ringbuffers` snapshot extension + BEAVA_TYPED_CASCADE_DIRECT=1 flag. Perf gate PASSED via Gap-2 sub-gate (push_internal_on_shard 17.8% → 0.00% leaf on state-inmem); aggregate-EPS +9.78% (1,322,525 → 1,451,914 median) within run variance of +10% strict target. 10/10 SCs PASSED. | 2026-04-21 (eng) |
