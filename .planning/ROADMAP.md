...
| 12.8 | Memory governance — cold-entity TTL + bucket-level reclaim + lifetime aggregation contract | Two-tier memory hygiene before final ship. **Tier 1 (entity-level):** opt-in cold-entity TTL eviction (default OFF — preserves "users size their box" stance from PROJECT.md; opt-in via per-source `@bv.event(cold_after=...)` or global `BEAVA_COLD_TTL`); `app.delete(entity_key)` for explicit removal. **Tier 2 (bucket-level — always on):** windowed-op state reclaims aged-out buckets within active entities (windowed ops have a 64-bucket cap per CLAUDE.md; trailing buckets that have rolled past their window are dead memory until next event triggers cleanup — make this active reclaim). **Lifetime aggregation contract:** explicit `lifetime=True` SDK kwarg semantics for `[first_seen, now)` forever-window — distinct from omitting `windowed=...` (which is op-dependent and ambiguous today); each of the 54 ops declares a finite per-entity memory ceiling for lifetime mode (O(1) for counters/sums; bounded sketch for HLL/DDSketch/SpaceSaving; bounded by `n` for first_n/last_n/most_recent_n; bounded by config for histogram/event_type_mix; register-time rejection for any op without a memory-bound proof). Eviction-aware contract: cold-evicted entity that receives new event resurrects with FRESH state (Redis TTL pattern; locked permanent). Metrics: `cold_entity_evictions_total`, `lifetime_op_cap_hit_total{op=...}`, `entity_count_resident`, `bytes_per_entity_p99`, `bucket_reclaim_total{op=...}`. **Out of scope:** SSD overflow / tiered eviction (locked permanently); resurrection with persisted history (would need disk; deferred); new operators beyond declaring lifetime semantics on existing 54. | 9 | 📋 **PLANNED 2026-05-01** — 9 plans across 5 waves (PLAN.md created); next on v0 critical path; runs before final ship phase 13 |
...
**Total active phases:** 20 (post-2026-04-30 v0-events-only commitment + Phase 12.8 memory governance inserted 2026-05-01). v0 ship critical path = **Phase 12.8 (memory governance — cold-entity TTL + bucket reclaim + lifetime contract) → Phase 13 (final v0 ship — SDK polish + benchmarks + ship)**. Phase 12.7 (table strip) ✅ CLOSED 2026-05-01.
...
- **Phase 12.8 inserted 2026-05-01** (memory governance — opt-in cold-entity TTL + always-on bucket-level reclaim within active entities + lifetime aggregation contract for `[first_seen, now)` forever-window semantics; runs before Phase 13 final ship)
...
Plans:
- [ ] 12.8-01-PLAN.md — Wave 1 — 4th JSON-prelude shim `pre_check_unbounded_op_in_lifetime_mode` in `register_validate.rs` + env-gate `BEAVA_MEMORY_GOV_ENFORCE` (Wave 1 default OFF; Plan 04 flips to ON) — placeholder helper returns Unbounded for every input
- [ ] 12.8-02-PLAN.md — Wave 1 — `cold_after` kwarg on Python `@bv.event` decorator + `cold_after_ms: Option<u64>` field on Rust `EventDescriptor` (range [1s, 365d] enforced at decoration; round-trips through serde + wire JSON)
- [ ] 12.8-03-PLAN.md — Wave 2 — Cold-entity TTL eviction on apply hot path (`apply_shard.rs::dispatch_push_sync` reads `descriptor.cold_after_ms`; FRESH-state-on-resurrect via `evict_entity_by_shape_if_cold` in `agg_state_table.rs` + last_seen_ms sidecar HashMaps) — depends on Plans 01 + 02
- [ ] 12.8-04-PLAN.md — Wave 2 — 54-row classification table populates `lifetime_bound_for_op_str` (`O1` / `BoundedSketch` / `BoundedByRequiredKwarg(<kwarg>)` / `BoundedByConfig(<kwarg>, <default>)`) + `agg_compile.rs` histogram cap-where-missing (env-gate flip moved to Plan 06 for wave-ownership) — depends on Plan 01
- [ ] 12.8-05-PLAN.md — Wave 2 — Architectural test `phase12_8_lifetime_ops_have_bounds.rs` (single-file, GREEN-by-default; coverage check that every op-string in agg_compile has a classification in register_validate; sister to 12.6/12.7 architectural-test pairs) — depends on Plan 01
- [ ] 12.8-06-PLAN.md — Wave 3 — 5 Prometheus metric families on `/metrics` admin sidecar (`beava_cold_entity_evictions_total` / `beava_lifetime_op_cap_hit_total` / `beava_entity_count_resident` / `beava_bucket_reclaim_total` / `beava_bytes_per_entity_p99`; UNLABELED v0; per-source labels deferred to v0.0.x) + flip `BEAVA_MEMORY_GOV_ENFORCE` env-gate default ON in `apply_shard.rs::memory_gov_enforce_enabled` (escape hatch: `=0`; moved here from Plan 04 for wave-2 file-ownership) — depends on Plans 03 + 04
