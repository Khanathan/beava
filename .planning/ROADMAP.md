...
**Total active phases:** 19 (post-2026-04-30 v0-events-only commitment). v0 ship critical path = Phase 12.7 (table strip) → Phase 13 (SDK polish + benchmarks + ship).
...
- **Phase 12.7 inserted 2026-04-30** (table strip — `@bv.table` + temporal store + retraction stripped per `project_v0_events_only_scope`)
...
### Phase 12.7: v0 table strip — events-only commitment — 📋 PLANNED 2026-05-01

**Status:** 📋 PLANNED 2026-05-01. 10 plans across 4 waves. CONTEXT.md captured 2026-05-01 (4 decisions D-01..D-04 locked: D-01 reset FORMAT_VERSION 2→1; D-02 hard errors framed "not supported in v0"; D-03 two-file architectural-test pair; D-04 comprehensive REQUIREMENTS sweep + 11.5 retro-descope). PATTERNS.md mapped 26 of 28 file analogs to 12.6 plans (every concrete deletion has a 12.6 precedent — JSON-prelude shim from Plan 04, architectural-test scaffold from Plan 10, banner-stamp from Plan 09, schema-version reset adapted from Plan 06's bump). Ready for `/gsd-execute-phase 12.7`.
...
- [ ] 12.7-08-PLAN.md — Wave 3 — Phase 11.5 retroactive-descope banner stamps on 3 files (D-04 second half); pattern from 12.6 Plan 09's 12.5 banner-stamp
- [ ] 12.7-09-PLAN.md — Wave 4 — Microbench rerun (3 cells) + throughput rebaseline (8 cells) + verdict logging (Phase 8+ contract)
- [ ] 12.7-10-PLAN.md — Wave 4 — Phase 12.7 SUMMARY + VERIFICATION + CLAUDE.md `§ Events-Only Invariant` block + STATE/ROADMAP/CORRECTNESS-PATH closure (advance v0 critical-path → Phase 13); section-ownership contract per 12.6 Plans 09 + 13
