...
- [ ] **V0-GLOBAL-AGG-01**: Engine sentinel routing for global tables — sentinel `entity_id = ""` (empty string) routes global-table state through the same per-entity hashmap machinery (no special-case branch; the existing `&str` key path handles `""` natively). Register-time validation accepts `key_cols: []` (empty array) as a valid global-table declaration; `key_cols` MUST be either non-empty (per-entity) or empty (global) — never null. Per [ADR-003](decisions/ADR-003-global-aggregation-and-bv-lit.md). Implementation: Phase 13.4 `crates/beava-server/src/apply_shard.rs::dispatch_*_sync` (no code change beyond removing any rejection of empty entity_id) + `crates/beava-core/src/register_validate.rs` (accept `key_cols: []`). Acceptance gate: `python/tests/v0/test_global.py` (Plan 13.0-16, 8 tests gated by `_engine_available()` SKIP until 13.4 + 13.5 land).
- [ ] **V0-GLOBAL-AGG-02**: SDK no-key form for global aggregation — `@bv.table` decorator without `key=` kwarg declares a global table; `events.group_by()` empty allowed (returns GroupBy with empty `_keys` tuple); `events.agg(**aggs)` shorthand on EventSource / EventDerivation (equivalent to `events.group_by().agg(**aggs)`); `App.get(table_name)` 1-arg form returns the global feature dict (cold-start `{}`). Mismatched arity (`app.get(global_table, "alice")` or `app.get(per_entity_table)`) raises `KeyError` with a clear message. Per [ADR-003](decisions/ADR-003-global-aggregation-and-bv-lit.md). Implementation: Phase 13.5 (Python ~110 LOC: `python/beava/_events.py` group_by allowance + agg shorthand + decorator + App.get overload) + Phase 13.6 (TS overload + Go separate `app.GetGlobal(tableName)` method per Go's typing convention; ~150 LOC across both). Acceptance gate: `python/tests/v0/test_global.py` (Plan 13.0-16).
- [ ] **V0-LIT-01**: Public `bv.lit(value)` expression literal export — promote the existing internal `_Literal` AST node (`python/beava/_col.py:195`) to the public `bv` namespace as a factory function `bv.lit(value: int | float | str | bool | None) → Expr`. Mirrors TypeScript `bv.lit(value)` + Go `bv.Lit(value any)`. Use cases: constant columns (`events.with_columns(source=bv.lit("web"))`); explicit type-coercion (`bv.col("a") / bv.lit(10.0)` for forced float division); compositions where implicit operator-overloading coercion is ambiguous. No wire-format change required (literals were already serialized via the existing operator-overloading path; `_Literal.to_expr_string()` produces the canonical JSON expression text). Per [ADR-003](decisions/ADR-003-global-aggregation-and-bv-lit.md). Implementation: Phase 13.5 (`python/beava/__init__.py` ~5 LOC) + Phase 13.6 (TS + Go SDK ~30 LOC each). Acceptance gate: `python/tests/v0/test_lit.py` (Plan 13.0-16, 5 tests).
