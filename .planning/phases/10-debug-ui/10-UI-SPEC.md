...
Base unit: **4px**. All spacing MUST be a multiple of 4 and drawn from the
standard set `{4, 8, 16, 24, 32, 48, 64}`. 12px is intentionally excluded.
...
:root {
  ...
  --space-2: 8px;   /* tight internal padding, row vertical padding */
  --space-4: 16px;  /* default content padding, horizontal input/button padding */
...
Token `--space-3` is intentionally omitted to keep the scale aligned with
`{4, 8, 16, 24, 32, 48}`. Do NOT reintroduce a 12px token.
...
| Header | `var(--space-2) var(--space-5)` (8px vertical, 24px horizontal) | `var(--space-5)` between wordmark and tabs |
| Tab bar | `0 var(--space-5)` | `var(--space-1)` between tabs |
| Tab button | `var(--space-2) var(--space-4)` (8px × 16px) | — |
| Main content | `var(--space-5)` (24px) | `var(--space-5)` between cards |
| Card / panel | `var(--space-5)` (24px) | `var(--space-4)` between inner sections |
| Stream list row | `var(--space-2) var(--space-4)` (8px × 16px) | `var(--space-4)` between cells |
| Form input | `var(--space-2) var(--space-4)` (8px × 16px) | `var(--space-2)` label-to-input |
| Metric key/value pair | `0` | `var(--space-2)` label-to-value |
| Memory bar row | `var(--space-2) 0` | `var(--space-4)` label-to-bar |
...
- **Touch targets:** The pause/refresh button and tab buttons MUST be at
  least **36px tall** on screen. With 14px text at line-height 1.5 (≈21px
  rendered height) and 8px vertical padding on both sides, the total is
  ~37px — meeting the 36px minimum. Stream list rows and form inputs are
  given explicit `min-height: 40px` so that 8px vertical padding still
  produces a comfortable desktop row. This is a debug UI for desktop, so
  36px is sufficient (not the mobile 44px standard).
...
|-------|------|-------------|--------|-----|
| `--text-xs` | 12px | 1.4 (16.8px) | 400 | Captions, timestamps, table headers, form labels |
| `--text-sm` | 14px | 1.5 (21px) | 400 | Body text, buttons, tab labels, code, mono keys, inline metric values, form inputs |
| `--text-xl` | 20px | 1.3 (26px) | 600 | Card titles, H2, wordmark |
| `--text-2xl` | 24px | 1.2 (28.8px) | 600 | Page H1, hero metric values |

Exactly four sizes: **12, 14, 20, 24**. There is no 16px token. Mono and
sans share the 14px body slot — mono is differentiated by font-family,
not size.
...
| Metric numeric value (large / hero) | 24px | 600 | mono | `--text-primary` |
| Metric numeric value (inline) | 14px | 400 | mono | `--text-primary` |
| Table header | 12px | 400 | sans (uppercase, 0.05em tracking) | `--text-tertiary` |
| Code / keys / stream names | 14px | 400 | mono | `--text-primary` |
| Form input text (mono) | 14px | 400 | mono | `--text-primary` |
...
This DAG canvas is the primary visual focal point of the Topology tab and
the first screen users see on load — it is what the eye lands on when the
Topology tab activates, and every other element on the tab (title, subtitle,
detail panel) supports it.
...
**Row styles:**
- Min-height: 40px (content plus 8px × 16px padding; row renders at ≥40px which meets the 36px touch-target minimum).
...
- Always 14px mono, right-aligned.
...
- Height: 40px (explicit).
- Padding: `var(--space-2) var(--space-4)` (8 × 16).
- Font: **14px** `--font-mono` (keys are always mono; matches the body scale).
...
1. **Header strip** — 48px tall, horizontal flex:
   - Left: monospace key label in **14px** mono, copy-on-click icon button
     (see icon list §12). The icon button has `aria-label="Copy key to clipboard"`.
...
2. **Feature grid** — `display: grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap: var(--space-4);`.
   ...
   │ 42                      │  ← 24px mono, --text-primary (hero value)
   │ Transactions · live     │  ← 11px --text-tertiary
   └─────────────────────────┘
   ```
   - Cell padding: `var(--space-4)` (16px).
   - Cell bg: `--bg-elevated`, radius `--radius-sm`, no border.
   - Numeric hero values render at 24px mono. Any inline numeric values
     elsewhere in the cell (e.g. units, secondary counts) render at 14px mono.
   - Booleans render as `true`/`false` in `--status-ok` / `--status-error`.
   - Strings render in sans, truncated at 32 chars with an ellipsis + tooltip.
...
- Each block: 12px label (`--text-secondary`), 24px mono hero value (`--text-primary`), 11px caption (`--text-tertiary`).

**Bar list** (below summary):
- Row grid: `grid-template-columns: minmax(160px, 1.5fr) minmax(60px, 0.5fr) 3fr minmax(80px, 0.6fr);`
- Columns: stream name (14px mono) · key count (12px mono `--text-secondary`) · bar · size label (14px mono, right-aligned).
- Row height: 32px, vertical padding `var(--space-2)`.
- Row gap between label and bar: `var(--space-4)` (16px).
- Bar chart axis / tick labels (if rendered): 12px mono `--text-tertiary`.
...
**Size formatting:** human bytes (`1.2 MB`, `458 KB`, `12.0 B`), 14px mono, 1 decimal place.
...
└────────────────────────────┘
```
- Each section divider: `1px solid --border-subtle`, vertical padding `var(--space-4)` (16px).
- Feature rows: `display: grid; grid-template-columns: 1fr auto; gap: var(--space-4);` — name left (14px mono), type+window right in 12px `--text-secondary`.
...
- Full-width within `.tab-body`, height auto, padding `var(--space-2) var(--space-4)` (8px × 16px).
...
| Copy entity key to clipboard | (icon-only, `aria-label="Copy key to clipboard"`) |
...
- Heading: 14px / 600 / `--text-primary`, margin-top `var(--space-4)`.
...
- Icon-only buttons MUST carry an explicit `aria-label`. The enumerated set
  for Phase 10 is in §8.5 (close detail, copy key to clipboard).
...
| `copy` | 14 | entity result header (copy key button, `aria-label="Copy key to clipboard"`) |
...
```css
:root {
  /* spacing — standard set only: 4, 8, 16, 24, 32, 48 */
  --space-1: 4px;  --space-2: 8px;
  --space-4: 16px; --space-5: 24px; --space-6: 32px; --space-8: 48px;
  /* NOTE: --space-3 (12px) intentionally omitted */
...
- [x] Spacing scale declared (standard set {4,8,16,24,32,48}, §2)
...
- [x] Focal point declared for Topology tab (§7.1)
- [x] Icon-only buttons have explicit aria-labels (§8.5, §10)
...
Ready for `gsd-ui-checker` re-verification.
