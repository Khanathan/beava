# Beava Throughput Benchmark — Baseline

Captures the **before-FINDINGS** throughput numbers for Beava in its current state (single-threaded core, JSON payloads inside binary frames). These numbers become the baseline for v1.2 Performance milestone phases.

## What it measures

- **Sync PUSH throughput** — events/sec, p50/p95/p99 latency per event
- **Three pipeline shapes** — small (1 stream, 5 features), medium (2 streams + 1 view), large (3 streams + 2 views with cascade + fan-out)
- **Concurrent client scaling** — 1, 4, 8, 16 parallel SDK connections

## Running

```bash
# 1. Start Beava in release mode
cd /data/home/beava
export PATH="/data/home/.cargo/bin:$PATH"
cargo build --release
./target/release/beava &
BEAVA_PID=$!
sleep 1

# 2. Run the benchmark
cd benchmark/beava-throughput
python3 bench.py --events 100000 --clients 1 --pipeline medium

# 3. Profile with perf (run while bench is executing)
perf record -F 997 -g -p $BEAVA_PID -- sleep 10
perf report --stdio | head -50

# 4. Clean up
kill $BEAVA_PID
rm -f /data/home/beava/beava.snapshot.*
```

## Results location

Runs are written to `benchmark/beava-throughput/results/` as timestamped JSON files.
