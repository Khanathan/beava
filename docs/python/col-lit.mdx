...
Anywhere an event class or chain is in scope, `Cls.field` and `chain.field` are sugar for `bv.col("field")`.
...
# These two forms are equivalent on the wire:
bv.col("email") == bv.lit("a@b.com")
Click.email     == "a@b.com"

# On a chain, too:
checkout = Click.filter(Click.amount > 100)
checkout.email  # → bv.col("email")
```

Names starting with `_` are **not** intercepted — `Click._chain`, `repr(Click)`, pickle, and IDE introspection keep working as normal. Typos surface at register / push time, same as `bv.col("typo")`.
