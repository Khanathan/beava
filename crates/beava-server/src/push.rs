...
/// Wire-agnostic push execution. Used by both the HTTP handler and the
/// TCP `OP_PUSH` handler (Phase 8 folded scope) so logic is shared and the
/// idempotency cache + WAL + apply path stays single-source-of-truth.
pub async fn execute_push(app: &AppState, event_name: &str, body_bytes: &[u8]) -> PushOutcome {
...
pub async fn push_handler(
    ...
    match execute_push(&app, &event_name, &body_bytes).await {
