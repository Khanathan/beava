...
fn test_fraud_medium_estimate() {
    ...
    let est = estimator::estimate_memory("fraud", "medium")
        .expect("estimator must support fraud/medium");
...
fn test_adtech_small_estimate() {
    let est = estimator::estimate_memory("adtech", "small")
        .expect("estimator must support adtech/small");
    let predicted_mb = est.expected_rss_bytes / (1024 * 1024);
    assert!(predicted_mb < 500, "adtech/small predicted {predicted_mb} MB; ≤ 500 MB sanity");
}
...
fn test_ecommerce_large_estimate() {
    ...
    let predicted_mb = est.expected_rss_bytes / (1024 * 1024);
    assert!(predicted_mb >= 100, "large size should predict ≥ 100 MB; got {predicted_mb}");
}
